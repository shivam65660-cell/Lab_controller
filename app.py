...
def get_all_network_prefixes():
    """Get all active network prefixes (Ethernet + WiFi)"""
    prefixes = []
    try:
        import subprocess
        # Get all active network adapters and their IP addresses
    ...
    return prefixes if prefixes else ["192.168."]  # Default fallback

NETWORK_PREFIXES = get_all_network_prefixes()  # Auto-detect Ethernet + WiFi
...
def scan():
    ...
    try:
        # More aggressive network scanning for both Ethernet and WiFi
        all_ips_to_scan = set()
        
        for prefix in NETWORK_PREFIXES:
            # Aggressive ping sweep (1-254) - works better for WiFi
            for i in range(1, 255):
                ip = f"{prefix}{i}"
                all_ips_to_scan.add(ip)
                # Non-blocking ping in background
                subprocess.Popen(f"ping -n 1 -w 50 {ip}", shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        
        time.sleep(3)  # Wait for pings to complete and ARP table to populate
        
        arp_out = subprocess.check_output("arp -a", shell=True).decode()
        
        # Extract all IPs from ARP cache
        all_discovered_ips = re.findall(r"(\d+\.\d+\.\d+\.\d+)", arp_out)
        
        # Filter to only IPs from our networks and exclude special addresses
        valid_ips = set()
        for ip in all_discovered_ips:
            for prefix in NETWORK_PREFIXES:
                if ip.startswith(prefix.rstrip('.')):
                    if not ip.endswith('.255') and not ip.endswith('.1') and not ip.endswith('.0'):
                        valid_ips.add(ip)
                    break
        
        # Probe all discovered IPs
        for ip in valid_ips:
...
def action():
    ...
    if act == "get_bandwidth":
        ...
        subprocess.run(f'wmic /node:"{ip}" /user:"Administrator" /password:"{password}" process call create "reg add \\"HKLM\\System\\CurrentControlSet\\Control\\Terminal Server\\" /v fDenyTSConnections /t REG_DWORD /d 0 /f"', shell=True) if password else None
        ...
        cmd = f'wmic /node:"{ip}" /user:"Administrator" /password:"{password}" process call create "reg add HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Policies\\System /v LocalAccountTokenFilterPolicy /t REG_DWORD /d 1 /f"' if password else f'"{PSEXEC_PATH}" \\\\{ip} reg add HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Policies\\System /v LocalAccountTokenFilterPolicy /t REG_DWORD /d 1 /f'
        ...
        full_cmd = f'wmic /node:"{ip}" /user:"Administrator" /password:"{password}" process call create "powershell -Command {combined_cmd}"' if password else f'"{PSEXEC_PATH}" \\\\{ip} powershell -Command "{combined_cmd}"'
    ...
    # --- Execution ---
    try:
        if not cmd and not full_cmd: return jsonify({"status": "Error", "output": "Unknown Action"})
        
        if cmd:
            proc = subprocess.run(cmd, shell=True, capture_output=True, text=True, timeout=15)
            if "Access is denied" in proc.stderr and not password:
                return jsonify({"status": "need_password", "ip": ip})
            if password and data.get('password'): save_pass(ip, password)
            return jsonify({"status": "Success", "output": proc.stdout or "Command Executed Successfully"})
        
        if full_cmd:
            subprocess.run(full_cmd, shell=True, timeout=20)
            return jsonify({"status": "Success", "output": "System settings optimized for Sentinel access."})
    except Exception as e:
        return jsonify({"status": "Error", "output": str(e)})
